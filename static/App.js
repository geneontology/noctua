...
var MMEEditorInit = function(){
	...
	each(g.all_edges(),
	     function(edge){
		 var new_eedge = new edit_edge(edge.subject_id(),
					       edge.predicate_id(),
					       edge.object_id());
	...
	each(layout['virtual_nodes'],
		 ...
		 var vn = new edit_node(id, 'virtual');
	...
	each(layout['paths'],
		 ...
		 for(var ni = 0; ni < (nodes.length -1); ni++ ){
		     ...
		     var new_vedge = new edit_edge(sub_id, null, obj_id);
