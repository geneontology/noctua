...
////
//// : node barista.js --debug 0 --users /tmp/users.yaml --groups /tmp/groups.yaml --public http://localhost:3400 --self http://localhost:3400 --context minerva_local --repl 7887 --default-namespace minerva_public_dev
////
...
// Set the now basically required (until we can harmonize the API,
// fingers crossed) barista default namespace. This is only useful for
// the proxied /search endpoint.
var barista_default_namespace = argv['n'] || argv['default-namespace'];
if (barista_default_namespace) {
  ll('Barista operate with default namespace: "' + barista_default_namespace + '"');
} else {
  _die('Option (n|default-namespace) is required.');
}
...
if (barista_debug) {
  barista_debug = parseInt(barista_debug);
}
...
var BaristaLauncher = function () {
  ...
  ///
  /// This is a special use case generated by
  /// https://github.com/geneontology/noctua/issues/619, where there
  /// are some endpoints in Minerva that need to be exposed with no
  /// examination, modification, or listeners. The API proxy (normal
  /// OP) is above. This currently piggy-backs on that work in the
