...
var MMEnvInit = function (model_json, in_relations, in_token) {
  ...
  function _connect_with_edge(eedge) {
    ...
    var new_conn_args = {
      ...
      'overlays': [ // does not!?
        ["Label", {
          'label': openann_edge_str + '&nbsp;' + readable_rn,
          'location': 0.5,
          'cssClass': "aLabel",
          'id': 'label'
        }]
  ...
  function _on_clairvoyance_update(data) {
    ...
    if (top > (wY - cursor_buffer)) {
      top = wY - cursor_buffer;
    }
    if (left > (wX - cursor_buffer)) {
      left = wX - cursor_buffer;
    }
    if (top < cursor_spacer) {
      top = cursor_spacer;
    }
    if (left < cursor_spacer) {
      left = cursor_spacer;
    }
