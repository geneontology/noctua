...
function repaint_info(ecore, aid, info_div) {
  ...
  if (tanns && tanns.length === 1) {
    mtitle = tanns[0].value();
  }
...
function node_stack_object(enode, aid, ecore) {
  ...
  function _add_table_row(item, color, prefix, suffix) {
    ...
    if (prefix) {
      out_rep = prefix + out_rep;
    }
    if (suffix) {
      out_rep = out_rep + suffix;
    }
    var trstr = null;
    if (color) {
  ...
  each(inf_types, function (item) {
    _add_table_row(item, null, '[', ']');
  });
  // Editable types last.
  var std_types = enode.types();
  each(std_types, function (item) {
    _add_table_row(item);
  });
...
function sorted_relation_list(relations, aid) {
  ...
  var rels = relations.sort(function (a, b) {
    ...
    // boost.
    if (pr_a === 0 && a['relevant']) {
      pr_a = boost;
    }
    if (pr_b === 0 && b['relevant']) {
      pr_b = boost;
    }
